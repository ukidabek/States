...
namespace Utilities.States.Test
{
	public class StateLogicExecutor : IStateLogicExecutor
	{
		...
		private List<IOnFixUpdateLogic> m_fixUpdateLogic = new List<IOnFixUpdateLogic>();
		...
		public void SetLogicToExecute(IState state)
		{
			...
			m_fixUpdateLogic.AddRange(state.Logic.OfType<IOnFixUpdateLogic>());
		}
		...
		public void RemoveLogicToExecute(IState state)
		{
			...
			m_fixUpdateLogic = m_fixUpdateLogic.Except(state.Logic.OfType<IOnFixUpdateLogic>()).ToList();
		}
